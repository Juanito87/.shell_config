...
[ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
# section "down" slices
__promptline_wrapper "$(__promptline_venv)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }

  # close sections
  printf "%s" "${reset_bg}${sep}$reset$space"
}

function __promptline_venv {
...
function __promptline_down_prompt {
  local slice_prefix slice_empty_prefix slice_joiner slice_suffix
